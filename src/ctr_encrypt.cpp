#include "../include/ctr_encrypt.hpp"

ocle::CTR_ENCRYPT::CTR_ENCRYPT(cl::Platform *platform,
							   cl::Device *device) :
		CTR(platform, device)
{
...
void ocle::CTR_ENCRYPT::setBuffers() {
	if (iv.size() == 0)
		throw new std::runtime_error("Initial vector empty");

	if (iv.size() != key->size())
		throw new std::runtime_error("IV and key bit size error");
